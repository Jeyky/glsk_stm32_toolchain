...
int main(void)
{
	...
	while (1) {
		...
		//snprintf(buffer, sk_arr_len(buffer), "T=%.0fC A=%u",(float) temp, (unsigned int)adcval);
		snprintf(buffer, sk_arr_len(buffer), "T=%.3fC",(float) temp);
